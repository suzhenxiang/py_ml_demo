...
def ai_acc_xed(df9,ky0=5,fgDebug=True):
    ...
    sub=df9['y_test']-df9['y_pred']
    all = len(sub)
    dsum = len(sub[sub == 0])
    dacc=(dsum*1.0/all)*100;
    #1
...
def ai_dat_rd(fsr0,k0=1,fgPr=False):    
    ...
    y_train[ysgn]=y_train[ysgn].apply(np.round).astype(int)*k0
    y_test[ysgn]=y_test[ysgn].apply(np.round).astype(int)*k0
    #3
    if fgPr:
        print('\nx_train');print(x_train.tail())
        print('\nx_test');print(x_test.tail())
...
def mx_fun_call(df,xlst,ysgn,funSgn,yksiz=1,yk0=5,fgInt=False,fgDebug=False):
    ...
    df[ysgn]=df[ysgn].apply(np.round).astype(int)*yksiz
